...
The [editable Mermaid source](assets/architecture.mmd) and its [render configuration](assets/mermaid-config.json) generate this committed SVG with Mermaid CLI. The hot path stays in CockroachDB. The archival worker is disabled by default because it eventually deletes the hot copy; when explicitly enabled, it claims a durable lease, verifies immutable objects, publishes the archive route, and only then performs cleanup.
